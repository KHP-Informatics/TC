...
exp_drop3 <- function(t,fun_args){fun_args[1] + 1 - exp(fun_args[2]*t + fun_args[3])}

exp_drop2 <- function(t,fun_args){fun_args[1] + 1 - exp(fun_args[2]*t )}

exp_drop1 <- function(t,fun_args){30 + 1 - exp(fun_args[1]*t )}
...
simulateData2 <- function(num_time_series,time_of_visits,exp_drop_par=array(c(30,0.0005),dim=c(1,2)),num_clusters = 2,noise=2,shift_range=c(-3000,1000),dilation_sd=1,trajectory_types = "sigmoid",curve_signs = "positive"){
  ...
  for (i in 1:num_clusters){
    ...
    for (j in cluster_member){
      ...
      data[[j]] <- round(exp_drop2(time_of_visits[[ts]]+time_shifts[j],exp_drop_par[i,])+error)
...
temporalClustering <- function(data,tps,K=2,lambda=0,phi=exp_drop,phi_init=c(30,0.0004,0),max_iter=30,traj_range=c(-10000,10000)){
...
thetas <- array(c(30,30,30,30,30,0.0005,0.0003,0.0005,0.0006,0.0008,30,30,30,30,30,0.0005,0.0001,0.0001,0.001,0.001),dim=c(5,4))
...
file_name <- paste("new",iter,".RData",sep="")
