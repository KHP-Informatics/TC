...
for (sim_num in 2:5){
  ...
  for (iter in 1:100){
    ...
    tmp_table <- data.frame(iter=iter + (sim_num-2)*100,real_clusters=simRes@clusters[,"real_clusters"],est_clusters=simRes@clusters[,"est_clusters"],discrim_score=simRes@loss[,3])
